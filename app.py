...
tier_map = {"Simple": "fast", "Advanced": "powerful"}
...
if question:
    ...
    st.markdown(
        ...
        f'<div>Namespace: <span>{ns}</span></div>'
    ...
    if data.get("sources"):
        with st.expander(f"Sources ({len(data['sources'])} passages)"):
            for s in data["sources"]:
                score_pct = f"{s['score']*100:.1f}%" if s.get("score") else ""
    ...
    if tier != "powerful":
        st.caption("Want more detail? Switch to Advanced in the dropdown above.")
...
with st.sidebar:
    ...
    st.markdown(
        "**Simple** — Amazon Nova Pro\n"
        "~200-token chunks, K=20, hybrid search\n\n"
        "**Advanced** — Claude Opus 4.6\n"
        "~1000-token chunks, K=20, hybrid search"
    )
    st.markdown("---")
